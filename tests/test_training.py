...
def test_optional_mlp_training_and_loading(tmp_path) -> None:
    ...
    assert set(predictor.residual_scale_db) == {"clear", "rainy", "rain_hot"}
    assert all(value > 0 for value in predictor.residual_scale_db.values())
    assert math.isfinite(metrics["pearson_r"])
    assert [coefficient.shape for coefficient in predictor.coefficients] == [
        (len(FEATURE_NAMES), 64),
        (64, 64),
        (64, 64),
        (64, 1),
    ]
...
def test_partial_weather_calibration_is_rejected(tmp_path) -> None:
    model_path = tmp_path / "partial_calibration.npz"
    np.savez_compressed(
        model_path,
        feature_names=np.asarray(FEATURE_NAMES),
        scaler_mean=np.zeros(len(FEATURE_NAMES)),
        ...
        coef_0=np.zeros((len(FEATURE_NAMES), 1)),
        intercept_0=np.zeros(1),
        residual_weather_names=np.asarray(["clear"]),
        residual_weather_scales=np.asarray([4.0]),
    )
    with pytest.raises(ValueError, match="every weather state"):
        LearnedMarginPredictor(model_path)
