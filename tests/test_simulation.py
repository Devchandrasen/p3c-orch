...
def small_config() -> ProjectConfig:
    return ProjectConfig(
        simulation=SimulationConfig(
            ...
            clusters=3,
            users=9,
            uavs_per_swarm_min=2,
            uavs_per_swarm_max=3,
            cache_size=5,
            cache_capacity_mb=30.0,
            library_size=20,
            arrival_rate=1.0,
            deadline_min=2,
            deadline_max=4,
...
def test_every_method_is_deterministic_and_returns_bounded_metrics(method: str) -> None:
    ...
    assert all(
        ...
        if name not in {"regime", "method", "seed"}
    )
    for name in (
        "drop_rate",
        "outage_probability",
...
def test_experiment_writes_detailed_summary_and_metadata(tmp_path) -> None:
    ...
    assert len(rows) == (
        len(config.simulation.regimes)
        * len(config.simulation.methods)
        * len(config.simulation.seeds)
    )
    metadata = json.loads(paths["metadata"].read_text(encoding="utf-8"))
    assert metadata["protocol"]["paired_seeds"] is True
    assert metadata["protocol"]["bundled_results"] is False
    assert metadata["protocol"]["results_origin"] == "generated_by_this_run"
...
@pytest.mark.parametrize("method", ["nearest", "rate-max", "random", "mucco-like"])
...
def test_unknown_regime_is_rejected() -> None:
    with pytest.raises(ValueError, match="unsupported regime"):
        SimulationRunner(small_config()).run("p3c-lr", 7, "unknown")
